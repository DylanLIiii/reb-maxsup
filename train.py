...
def max_suppression(logits: torch.Tensor, epoch: int, begin_lambda: float = 0.1, end_lambda: float = 0.2, epochs: int = 100) -> torch.Tensor:
    """
    Return the loss term after applying max_suppression
    """
    smoothing = begin_lambda + (end_lambda - begin_lambda) * epoch / epochs
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, model_ema=None, scaler=None):
    ...
    header = f"Epoch: [{epoch}] Using {which_training_mode(args)}"
